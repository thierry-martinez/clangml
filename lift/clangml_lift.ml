...
class lift_expr loc = object(self)
  ...
  method! open_node :
    'a 'b . ('a -> Parsetree.expression) -> ('b -> Parsetree.expression) ->
      ('a, 'b) Clang.Ast.open_node -> Parsetree.expression = fun _a _qual_type node ->
    [%expr {
  ...
  method! qual_type (qual_type : Clang.Ast.qual_type) =
    ...
    [%expr {
      cxtype = Clang__bindings.get_cursor_type (Clang__bindings.get_null_cursor ());
...
class lift_pattern loc = object(self)
  ...
  method! open_node :
    'a 'b . ('a -> Parsetree.pattern) -> ('b -> Parsetree.pattern) ->
      ('a, 'b) Clang.Ast.open_node -> Parsetree.pattern = fun _a _qual_type node ->
    [%pat? {
      decoration = _;
      desc = [%p _a node.desc] }]
