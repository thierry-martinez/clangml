...
external reparse_translation_unit :
  cxtranslationunit -> cxunsavedfile array -> Cxreparse_flags.t -> bool =
...
type cxcursorkind =
  ...
  | ObjCSelfExpr
  [@ocaml.doc "Represents the \"self\" expression in a ObjC method."]
  | UnexposedStmt
  [@ocaml.doc
...
type cxtypekind =
  | Invalid
  [@ocaml.doc "Reprents an invalid type (e.g., where no type is available)."]
  | Unexposed
  [@ocaml.doc
...
[@@ocaml.doc "Retrieve the result type associated with a function type."]
...
[@@ocaml.doc "Retrieve the type of an argument of a function type."]
...
external cursor_is_dynamic_call :
  cxcursor -> bool = "clang_Cursor_isDynamicCall_wrapper"[@@ocaml.doc
                                                           "Given a cursor pointing to a C++ method call or an ObjC message, returns non-zero if the method/message is \"dynamic\", meaning:"]
...
external cursor_is_obj_coptional :
  cxcursor -> bool = "clang_Cursor_isObjCOptional_wrapper"[@@ocaml.doc
                                                            "Given a cursor that represents an ObjC method or property declaration, return non-zero if the declaration was affected by \"\". Returns zero if the cursor is not such a declaration or it is \"\"."]
...
type cxcomment[@@ocaml.doc "A comment AST node."]
...
type clang_ext_attrkind =
  ...
  | Mode 
  | OpenCLImageAccess 
  | Overloadable [@@deriving (eq, ord, show)]
